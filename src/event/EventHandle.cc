...
void EventHandle::ReconstructEventFromBuffer()
{
    ...
    for (auto i = 0; i < _buffer.num_protons; ++i)
    {
        protons.emplace_back(_buffer.proton_xi[i], _buffer.proton_method[i], _buffer.proton_lhcSector[i], _buffer.proton_isValid[i]);
    }
    _event.SetProtons(std::move(protons));

    std::vector<LocalTrack> local_tracks{};
    for (auto i = 0; i < _buffer.num_local_tracks; ++i)
    {
        local_tracks.emplace_back(_buffer.local_track_rpId[i], _buffer.local_track_x[i], _buffer.local_track_x_unc[i], _buffer.local_track_y[i], _buffer.local_track_y_unc[i], _buffer.local_track_tx[i], _buffer.local_track_tx_unc[i], _buffer.local_track_ty[i], _buffer.local_track_ty_unc[i], _buffer.local_track_chiSquaredOverNDF[i]);
    }
    _event.SetLocalTracks(std::move(local_tracks));
}
